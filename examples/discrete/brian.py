...
class brian(player): # inherits from the player class 
    '''Runs IESDS and chooses randomly among surviving actions'''
    ...
    def play(self, state, DOPRINT=False): 
        ...
        # 2. select random action from surviving set of Brian's actions
        a_choice = np.random.choice(A_undominated[i])

        # 3. find the index in the *original* strategy set
        for ia,a in enumerate(A[i]): 
            if a == a_choice: 
                break
